...
После неоднозначной ошибки сначала проверяется ledger; произвольного
`migrate resolve`, отката прав или повторного списания нет. Отдельно допущено
восстановление exact failed attempt `412a6ec9-35c7-4c1a-ad94-b11dbae1e889`:
проверены нулевые applied steps, неизменные ACL и rollback транзакции.
Контроллер нормализует только `NOINHERIT` исторической runtime-роли при
нулевом числе memberships и записывает эту попытку как rolled-back штатным
Prisma CLI, сохраняя исходную запись. Иной ID/checksum/state блокирует выпуск.
