...
if [[ "$release_scope" =~ ^support-chat(-(activate|repair))?$ ]]; then
...
if [[ "$release_scope" == crm-customers-provider-config ]]; then
	[[ "$expected_crm_customers_provider_baseline_sha256" =~ ^[a-f0-9]{64}$ ]] || die 'Customers provider configuration requires its exact baseline hash.'
else
	[[ -z "$expected_crm_customers_provider_baseline_sha256" ]] || die 'Customers provider authorization cannot be reused by another scope.'
fi
...
if [[ "$release_scope" == crm-upgrade || "$release_scope" == crm-live-updates || "$release_scope" == crm-sales-runtime ]]; then
...
if [[ "$release_scope" == crm-sales-runtime ]]; then
	scoped_shell_file="$controller_root/scripts/deploy-crm-sales-runtime-scoped.sh"
	scoped_node_file="$controller_root/scripts/crm-sales-runtime.mjs"
elif [[ "$release_scope" == crm-live-updates ]]; then
...
elif [[ "$release_scope" == crm-prepare || "$release_scope" == crm-databases || "$release_scope" == crm-runtime || "$release_scope" == crm-upgrade ]]; then
	...
	scoped_shell_file="$controller_root/scripts/deploy-billing-crm-acl-scoped.sh"
	scoped_node_file="$controller_root/scripts/billing-crm-acl-release.mjs"
...
elif [[ "$release_scope" =~ ^support-chat(-(activate|repair))?$ ]]; then
...
for scoped_file in "$scoped_shell_file" "$scoped_node_file"; do
	...
	scoped_file_limit=131072
	# The shared verifier includes the exact historical owner-ledger proofs.
	# Keep every shell and other standalone Node payload at its existing limit.
	if [[ "$scoped_file" == "$controller_root/scripts/scoped-service-release.mjs" ]]; then scoped_file_limit=147456; fi
	(( scoped_file_size > 0 && scoped_file_size <= scoped_file_limit )) ||
...
if [[ "$release_scope" == crm-sales-runtime ]]; then
	command -v node >/dev/null || die 'CRM Sales payload packaging requires Node.js.'
	for scoped_name in crm-sales-runtime.mjs crm-live-release.mjs scoped-service-release.mjs; do
		git -C "$controller_root" ls-files --error-unmatch "scripts/$scoped_name" >/dev/null 2>&1 || die 'CRM Sales module is not tracked by immutable Infra.'
	done
	scoped_envelope="$(node "$scoped_node_file" pack "$controller_root/scripts")" || die 'Cannot package the bounded CRM Sales payload.'
	scoped_node_sha256="$(printf '%s' "$scoped_envelope" | sha256sum | awk '{print $1}')"
	scoped_node_base64="$(printf '%s' "$scoped_envelope" | gzip -n -6 -c | base64 | tr -d '\n')"
	unset scoped_envelope
fi
...
if [[ "$release_scope" == gateway-tilda-upgrade ]]; then
	...
	for scoped_name in scoped-service-release.mjs gateway-tilda-release.mjs; do
		git -C "$controller_root" ls-files --error-unmatch "scripts/$scoped_name" >/dev/null 2>&1 || die 'Gateway module is not tracked by immutable Infra.'
	done
	scoped_envelope="$(node "$scoped_node_file" pack)" || die 'Cannot package the bounded Gateway payload.'
	scoped_node_sha256="$(printf '%s' "$scoped_envelope" | sha256sum | awk '{print $1}')"
	scoped_node_base64="$(printf '%s' "$scoped_envelope" | gzip -n -6 -c | base64 | tr -d '\n')"
	unset scoped_envelope
fi
if [[ "$release_scope" == crm-commerce-activate ]]; then
	command -v node >/dev/null || die 'Commerce payload packaging requires Node.js.'
	for scoped_name in crm-commerce-activation-cli.mjs crm-commerce-activation.mjs crm-commerce-database.mjs crm-release.mjs scoped-service-release.mjs; do
		git -C "$controller_root" ls-files --error-unmatch "scripts/$scoped_name" >/dev/null 2>&1 || die 'Commerce module is not tracked by immutable Infra.'
	done
	scoped_envelope="$(node "$scoped_node_file" pack)" || die 'Cannot package the bounded commerce payload.'
	scoped_node_sha256="$(printf '%s' "$scoped_envelope" | sha256sum | awk '{print $1}')"
	scoped_node_base64="$(printf '%s' "$scoped_envelope" | gzip -n -6 -c | base64 | tr -d '\n')"
	unset scoped_envelope
fi
...
if [[ "$release_scope" == crm-customers-provider-config ]]; then
	for scoped_name in crm-customers-provider-config.mjs crm-commerce-activation.mjs crm-release.mjs scoped-service-release.mjs; do
		git -C "$controller_root" ls-files --error-unmatch "scripts/$scoped_name" >/dev/null 2>&1 || die 'Provider module is not tracked by immutable Infra.'
	done
	scoped_envelope="$(node "$scoped_node_file" pack)" || die 'Cannot package the bounded provider payload.'
	scoped_node_sha256="$(printf '%s' "$scoped_envelope" | sha256sum | awk '{print $1}')"
	scoped_node_base64="$(printf '%s' "$scoped_envelope" | gzip -n -6 -c | base64 | tr -d '\n')"
	unset scoped_envelope
fi
...
if [[ "$release_scope" != all || -f "$release_root/apps/operations/prisma/migrations/20260910110000_remove_admin_backlog/migration.sql" ]]; then
	...
	cleanup_scoped_payload() {
		if [[ "${release_scope:-all}" == crm-commerce-activate ]]; then
			local name
			for name in crm-commerce-activation-cli.mjs crm-commerce-activation.mjs crm-commerce-database.mjs crm-release.mjs scoped-service-release.mjs; do
		...
		if [[ "${release_scope:-all}" == crm-customers-provider-config ]]; then
			local name
			for name in crm-customers-provider-config.mjs crm-commerce-activation.mjs crm-release.mjs scoped-service-release.mjs; do
				rm -f -- "$scoped_payload_directory/$name"
			done
		fi
		...
		if [[ "${release_scope:-all}" == crm-sales-runtime ]]; then
			rm -f -- "$scoped_payload_directory/crm-sales-runtime.mjs" "$scoped_payload_directory/crm-live-release.mjs" "$scoped_payload_directory/scoped-service-release.mjs"
		fi
		...
		if [[ "${release_scope:-all}" =~ ^support-chat(-(activate|repair))?$ ]]; then
	...
	scoped_decode_payload() {
		...
		# Mirror the immutable source-selection map: these scopes use only
		# scoped-service-release.mjs as their standalone verifier. This does not
		...
		printf '%s' "$encoded" | base64 --decode | gzip -dc | head -c "$((limit + 1))" >"$destination" ||
	...
	if [[ "${release_scope:-all}" =~ ^support-chat(-(activate|repair))?$ ]]; then
	...
	elif [[ "${release_scope:-all}" == identity-api-runtime || "${release_scope:-all}" == identity-email-delivery || "${release_scope:-all}" == crm-live-updates || "${release_scope:-all}" == crm-sales-runtime ]]; then
		...
		if [[ ! "$scoped_envelope_size" =~ ^[0-9]+$ ]] || (( scoped_envelope_size <= 0 || scoped_envelope_size > 262144 )); then
			die 'Identity envelope exceeds its decoded limit.'
		fi
		...
		# Shells/private envs/other standalone Node files remain <= 128 KiB;
		# the selected standalone scoped-service-release.mjs is <= 144 KiB.
		# Other scopes retain the existing decoder and 90000-byte SSH budget.
		printf '%s' "$scoped_node_base64" | base64 --decode | gzip -dc | head -c 524289 >"$scoped_payload_directory/verifier.mjs" || die 'Commerce envelope decompression failed.'
		scoped_envelope_size="$(wc -c <"$scoped_payload_directory/verifier.mjs" | tr -d '[:space:]')"
		...
		(( scoped_envelope_size > 0 && scoped_envelope_size <= 524288 )) || die 'Reminders envelope exceeds its decoded limit.'
	elif [[ "${release_scope:-all}" == crm-customers-provider-config ]]; then
		printf '%s' "$scoped_node_base64" | base64 --decode | gzip -dc | head -c 524289 >"$scoped_payload_directory/verifier.mjs" || die 'Provider envelope decompression failed.'
		scoped_envelope_size="$(wc -c <"$scoped_payload_directory/verifier.mjs" | tr -d '[:space:]')"
		[[ "$scoped_envelope_size" =~ ^[0-9]+$ ]] || die 'Provider envelope size is invalid.'
		(( scoped_envelope_size > 0 && scoped_envelope_size <= 524288 )) || die 'Provider envelope exceeds its decoded limit.'
	else
