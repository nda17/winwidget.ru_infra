...
import { sha256, validateRestoreEvidence } from './scoped-service-release.mjs';

const flags = new Map();
for (let index = 2; index < process.argv.length; index += 2) {
	const key = process.argv[index];
	assert.ok(['--artifact', '--artifact-sha256', '--phase-a', '--migration', '--image', '--output'].includes(key));
	assert.ok(!flags.has(key) && process.argv[index + 1]);
	flags.set(key, process.argv[index + 1]);
}
assert.equal(flags.size, 6);
...
assert.equal(sha256(readFileSync(artifact)), artifactSha256);
...
const restore = database => docker(['exec', '-i', container, 'pg_restore', '--exit-on-error', '--single-transaction', '--no-owner', '--username', 'operations_rehearsal', '--dbname', database], readFileSync(artifact));
const snapshot = database => JSON.parse(sql(`
...
try {
	...
	evidence = {
		...
		artifactSha256, restoreImageId: image, postgresMajor: 18, restoreExitCode: 0,
